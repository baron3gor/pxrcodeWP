 ...
 */
if (!function_exists('pxr_init_widgets')) {
   function pxr_init_widgets()
   {
      register_widget('Pxrtheme_Latestposts_Widget');
      register_widget('Pxrtheme_Social');
      register_widget('Pxrtheme_Spacer_Widget');
   }
}

add_action('widgets_init', 'pxr_init_widgets');

/**
 * Image field for widgets 
 */
if (!class_exists('Pxrtheme_WidgetImageField')) {
   class Pxrtheme_WidgetImageField
   {
      private $image_id;
      private $src;
      private $width;
      ...
      function __construct($widget = null, $image_id = 0)
      {
         ...
         if ($uri && in_array($file, array('widgets.php')) && is_admin()) {
            ...
            wp_enqueue_script('widgetimagefield', '/assets/js/widgets.js', array('jquery', 'jquery-ui-core', 'thickbox', 'media-upload'), false, true);
